...
Mode_graph <- Mode_graph + labs(title = "Modes for cases",
...
# department tile
Dept_tile <-
  ggplot(data=subset(origin_case5, !is.na(Dept)), aes(y = Dept, x = next_dept)) +
  geom_tile(aes(fill=prob), color = "white") +
  # scale_fill_manual(values=colors) +
...
write.csv(origin_case4, file = "origin_case4.csv")
# # # new sample data
